...
const textRef = useRef<TextRefWithOpacity | null>(null);
    ...
    return (
        ...
        >
            ...
            {/* In-world profile card (coder / free code vendor) */}
            {id === "coder" && (
                <VendorProfileCard
                    vendorId="coder"
                    color={color}
...
function VendorWrapper(props: VendorWithShout) {
    ...
    const shoutOffset = "shoutBubbleOffset" in props && props.shoutBubbleOffset ? props.shoutBubbleOffset : [0.8, 1.9, 0.5] as [number, number, number];
    if (config) {
        return <RealisticVendorBody {...shared} config={config} shoutBubbleOffset={shoutOffset} />;
    }
    return <CyberHuman {...shared} />;
}
