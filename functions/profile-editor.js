...
export async function deleteBuilder() {
    ...
    // Delete in order: notes -> logs -> follows -> stars -> comments -> builder
    ...
    await supabase.from('follows').delete().eq('builder_id', builder.id);
    await supabase.from('stars').delete().eq('builder_id', builder.id);
